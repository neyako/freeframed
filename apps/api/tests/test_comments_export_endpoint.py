...
def test_nle_rejected_for_audio_but_csv_allowed(_, client, mock_db, auth_headers):
    ...
    # This fork hoists the media_file lookup above the format branch (see
    # comments.py export_comments — a deliberate adaptation vs upstream, which
    # queries it separately per-branch), so every path makes 3 .first() calls
    # (asset, version, media_file) even on this early-reject path.
    mock_db.first.side_effect = [asset, version, _media()]
    r = client.get(f"/assets/{asset.id}/comments/export?format=edl&version_id={version.id}",
                   headers=auth_headers)
    assert r.status_code == 422
