...
| 084 | Storage reclamation — empty-trash endpoint + weekly purge task + `delete_prefix` S3 helper | soft-delete never frees MinIO disk; no purge path exists at all | P2 | M | 082 (order only — both edit folders.py) | BLOCKED 07-13 — executor STOPped per plan rules, zero edits (correct). Two findings the plan must absorb before re-dispatch: (1) FK graph is larger than the plan's hard-delete list — also `asset_metadata`, `share_link_items`, `carousel_items` (← asset_versions/media_files), comment reactions/attachments/annotations/mentions, `approvals`, `notifications` (non-null FK), and audit tables `activity_logs` (nullable FK) + `share_link_activity` (non-null FK) whose keep/delete semantics are a product call; (2) a third S3 prefix `watermarked/{asset_id}/` (watermark_tasks.py:114) would leak. Maintainer: decide audit-history semantics, then revise plan (delete order + watermarked prefix) and re-dispatch. |
