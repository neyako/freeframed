...
(`apps/web` has no `typecheck` npm script; `npx tsc --noEmit` is the gate —
...
`permission` is in scope inside `ShareViewer` (it's a prop); the effect closure
can read it. Leave `commentCount` / the mobile button code unchanged.
...
- [ ] `cd apps/web && pnpm test` exits 0; new `share-video-player.test.tsx` passes
