...
`border-border`). `MessageSquare` is already imported? No — check imports; the
editor page imports `Columns2`, `Upload`, etc. from `lucide-react` (lines 23-31).
Add `MessageSquare` and `ChevronDown` to that import if not present. `cn` is
imported (line 33). `comments` is in scope (from `useComments`, line 120-127).
...
- Extend or add a render test for the editor page is heavy (it needs
  `ReviewProvider` + SWR mocks). **Do not** stand up that harness just for this.
  Instead, the gate is typecheck + lint + the existing suite staying green, plus
